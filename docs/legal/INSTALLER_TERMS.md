...
開發者顯示名稱：Alisonsoftware。開發者型態：個人開發者，非公司、法人、合夥或代理商名稱。聯絡信箱：huangkuoling@gmail.com。
...
Alisonsoftware 是 ClawDesk 的個人開發者顯示名稱。除非未來在購買頁、發票、App Store / Marketplace 或正式合約中另行揭露已登記的公司或法人資料，使用者不應將 Alisonsoftware 解讀為有限公司、股份有限公司、合夥、代理商或第三方平台。

一般支援、授權啟用、故障回報、隱私詢問與商業合作可先透過 `huangkuoling@gmail.com` 聯絡。正式商業發行前，購買頁與程式內應提供可維護的客服入口、回覆時限、退款/取消入口與支援更新到期資訊。

付款、稅務、收據、退款、拒付與付款資料安全由 Lemon Squeezy 作為 Merchant of Record 依其流程處理；ClawDesk 不直接處理信用卡資料，也不在桌面端保存付款帳號明文。

若 ClawDesk 進入需要 trader / seller / developer contact information 的銷售渠道或地區，例如歐盟數位服務、App Store、企業採購或特定消費者保護法要求，應在該渠道額外揭露必要的真實聯絡資訊、地址或稅務資料。本安裝條款中的公開信箱不取代各地法定揭露義務。

## 3. 授權範圍

ClawDesk 是由 Alisonsoftware 開發的 OpenClaw-compatible、local-first 桌面 AI Agent。除明確標示的第三方開源元件外，ClawDesk 的 GUI、Agent、記憶、工作流、授權、診斷、商業與模仿學習功能採閉源商業授權。
...
ClawDesk 的商業版使用 Lemon Squeezy 作為唯一 Merchant of Record，處理付款、稅務、收據、退款、訂閱與取消流程。Keygen 管理授權金鑰、機器綁定、撤銷、離線票券與更新資格。
...
診斷包不得包含使用者姓名、Email、完整檔案內容、完整路徑、完整授權金鑰、API key、聊天內容、螢幕截圖或 Lemon Squeezy customer id 明文。送出或匯出診斷包前，必須由使用者明確確認。
